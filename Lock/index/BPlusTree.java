...
public class BPlusTree {
    ...
    public Optional<RecordId> get(DataBox key) {
        ...
        for (DataBox k : node.getKeys()) {
            if (k.equals(key)) {
                return Optional.of(node.getRids().get(i));
            }
            i++;
        }
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        // TODO(hw4_part2): B+ tree locking
        LockUtil.ensureSufficientLockHeld(lockContext, LockType.X);
        Optional<Pair<DataBox, Long>> childReturn = root.put(key, rid);
        if (childReturn.isPresent()) {
            List<DataBox> keys = new ArrayList<>();
            List<Long> children = new ArrayList<>();

            keys.add(childReturn.get().getFirst());
            children.add(root.getPage().getPageNum());
            children.add(childReturn.get().getSecond());
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        LockUtil.ensureSufficientLockHeld(lockContext, LockType.X);
        while (data.hasNext()) {
            Optional<Pair<DataBox, Long>> childReturn = root.bulkLoad(data, fillFactor);
            if (childReturn.isPresent()) {
                List<DataBox> keys = new ArrayList<>();
                List<Long> children = new ArrayList<>();
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        public boolean hasNext() {
            ...
            while (sibling.isPresent()) {
                ...
                else sibling = curr.getRightSibling();
            }
            return false;
        }
