...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        *           then n is split into a left and right node and a pair (split_key, right_node_page_num)
        *           is returned where right_node_page_num is the page number of the newly
        *           created right node, and the value of split_key depends on
        *           whether n is an inner node or a leaf node.
        */
        List<DataBox> newNodeKeys = new ArrayList<>();
        List<RecordId> newNodeRids = new ArrayList<>();

        DataBox splitKey = keys.get(order);
        newNodeKeys = keys.subList(order, keys.size());
        keys = keys.subList(0, order - 1);

        newNodeRids = rids.subList(order, rids.size());
        rids = rids.subList(0, order - 1);

        LeafNode newNode = new LeafNode(metadata, bufferManager, newNodeKeys, newNodeRids, Optional.empty(), treeContext);
        rightSibling = Optional.of(newNode.getPage().getPageNum());
        sync();
        newNode.sync();
        Pair returnPair = new Pair(splitKey, rightSibling);
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        // TODO(hw2): implement
        if (keys.size() != 0)
            throw new BPlusTreeException("Bulkload only works on empty trees.");
        ...
        int order = metadata.getOrder();
        int overflowNum = (int)Math.ceil(fillFactor * order * 2 + 1);

        while (data.hasNext()) {
            Pair<DataBox, RecordId> curr = data.next();
            keys.add(curr.getFirst());
            Collections.sort(keys);
            rids.add(keys.indexOf(curr.getFirst()), curr.getSecond());
            sync();
            if (keys.size() < overflowNum) {
                continue;
            } else {
                List<DataBox> new_node_keys = new ArrayList<>();
                List<RecordId> new_node_rids = new ArrayList<>();

                DataBox splitKey = keys.get(overflowNum - 1);
                new_node_keys.add(splitKey);
                keys = keys.subList(0, overflowNum - 1);
                new_node_rids.add(rids.get(keys.indexOf(splitKey)));
                rids = rids.subList(0, overflowNum - 1);
                LeafNode newNode = new LeafNode(metadata, bufferManager, page, new_node_keys, new_node_rids, Optional.empty(), treeContext);
                rightSibling = Optional.of(newNode.getPage().getPageNum());
                sync();
                newNode.sync();
                Pair returnPair = new Pair(splitKey, rightSibling);
