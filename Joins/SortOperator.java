...
public class SortOperator {
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        // TODO(hw3_part1): implement
        PriorityQueue<Pair<Record, Integer>> elem = new PriorityQueue<>(new RecordPairComparator());
        List<Iterator<Record>> runIters = new ArrayList<>();
        Run newRun = createRun();
        for (int i = 0; i < runs.size(); i++) {
            runIters.add(i, runs.get(i).iterator());
            if (runIters.get(i).hasNext()) {
                elem.add(new Pair<>(runIters.get(i).next(), i));
            }
        }
        while (!elem.isEmpty()) {
            Pair<Record, Integer> next = elem.poll();
            newRun.addRecord(next.getFirst().getValues());
            if (runIters.get(next.getSecond()).hasNext()) {
                elem.add(new Pair<>(runIters.get(next.getSecond()).next(), next.getSecond()));
            }
        }
        return newRun;
    }
    ...
    public List<Run> mergePass(List<Run> runs) {
        ...
        for (int i = 0; i < runs.size(); i += numBuffers - 1) {
            newRun.add(mergeSortedRuns(runs.subList(i, Math.min(runs.size(), i + numBuffers - 1))));
        }
        return newRun;
    }
    /**
