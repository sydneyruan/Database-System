...
public class ARIESRecoveryManager implements RecoveryManager {
    ...
    public long commit(long transNum) {
        // TODO(hw5): implement
        Long num = transactionTable.get(transNum).lastLSN;
        CommitTransactionLogRecord rec = new CommitTransactionLogRecord(transNum, num);
        logManager.appendToLog(rec);
        logManager.flushToLSN(num);
    ...
    public long abort(long transNum) {
        // TODO(hw5): implement
        Long num = transactionTable.get(transNum).lastLSN;
        AbortTransactionLogRecord rec = new AbortTransactionLogRecord(transNum, num);
        logManager.appendToLog(rec);
    ...
    public long end(long transNum) {
        // TODO(hw5): implement
        Long num = transactionTable.get(transNum).lastLSN;
        EndTransactionLogRecord rec = new EndTransactionLogRecord(transNum, num);
        logManager.appendToLog(rec);
    ...
    public long logPageWrite(long transNum, long pageNum, short pageOffset, byte[] before,
        ...
        transEntry.transaction.setStatus(Status.RUNNING);
        long lsn = transEntry.lastLSN;
        if (!transEntry.touchedPages.contains(pageNum))
            transEntry.touchedPages.add(pageNum);

        // If the number of bytes written is too large
        if (before.length > BufferManager.EFFECTIVE_PAGE_SIZE / 2) {
            UpdatePageLogRecord undo = new UpdatePageLogRecord(transNum, pageNum, lsn, pageOffset, null, after);
            Long undoLsn = logManager.appendToLog(undo);
            if (!dirtyPageTable.containsKey(pageNum))
                dirtyPageTable.put(pageNum, undoLsn);
            UpdatePageLogRecord redo = new UpdatePageLogRecord(transNum, pageNum, undoLsn, pageOffset, after, null);
            transEntry.lastLSN = logManager.appendToLog(redo);
        }
        else {
            UpdatePageLogRecord both = new UpdatePageLogRecord(transNum, pageNum, lsn, pageOffset, before, after);
            transEntry.lastLSN = logManager.appendToLog(both);
            if (!dirtyPageTable.containsKey(pageNum))
                dirtyPageTable.put(pageNum, both.getLSN());
        }
