...
public class LockManager {
    ...
    public boolean checkCompatibility(ResourceName name, LockType lockType) {
    ...
    throws DuplicateLockRequestException, NoLockHeldException {
        ...
        synchronized (this) {
            ...
            boolean isCompatible = checkCompatibility(name, lockType);
    ...
    throws NoLockHeldException {
        ...
        synchronized (this) {
            ...
            if (!entry.waitingQueue.isEmpty()) {
                LockRequest request = entry.waitingQueue.getFirst();
                Lock lock = request.lock;
                boolean isCompatible = checkCompatibility(lock.name, lock.lockType);
                if (promote && !isCompatible) {
                    promote(request.transaction, lock.name, lock.lockType);
    ...
    throws DuplicateLockRequestException, NoLockHeldException, InvalidLockException {
        ...
        synchronized (this) {
            boolean isCompatible = checkCompatibility(name, newLockType);
            blocked = !isCompatible;
            Lock l = getLock(name, transaction);
            ...
            // Check if TRANSACTION has lock on NAME, and set to new lock type
            if (!blocked) {
